                   ...
                   const solopt_t *opt)
{
    ...
    p+=sprintf(p,"%s%s%14.4f%s%14.4f%s%14.4f%s%14.4f%s%14.4f%s%14.4f%s%3d%s%3d%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%8.4f"
                 "%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%8.4f%s%6.2f%s%6.1f\n",
               s,sep,sol->rr[0],sep,sol->rr[1],sep,sol->rr[2],sep,sol->rr[3],sep,
               sol->rr[4],sep,sol->rr[5],sep, sol->stat,sep,sol->ns,sep,SQRT(sol->qr[0]),sep,SQRT(sol->qr[1]),sep,SQRT(sol->qr[2]),
               sep,sqvar(sol->qr[3]),sep,sqvar(sol->qr[4]),sep,sqvar(sol->qr[5]),
               sep,SQRT(sol->qvr[0]),sep,SQRT(sol->qvr[1]),sep,SQRT(sol->qvr[2]),
               sep,sqvar(sol->qvr[3]),sep,sqvar(sol->qvr[4]),sep,sqvar(sol->qvr[5]),
...
extern int outsolheads(unsigned char *buff, const solopt_t *opt)
{
    ...
    if (opt->posf==SOLF_LLH) { /* lat/lon/hgt */
        ...
        p+=sprintf(p,"%14s%s%14s%s%14s%s%14s%s%14s%s%14s%s%3s%s%3s%s%8s%s%8s%s%8s%s%8s%s%8s%s%8s%s%8s%s%8s%s%8s%s%8s%s%8s%s%8s%s%6s%s%6s\n",
                   "x-ecef(m)",sep,"y-ecef(m)",sep,"z-ecef(m)",sep,"x-vel(m/s)",
                   sep,"y-vel(m/s)",sep,"z-vel(m/s)",sep,"Q", sep,"ns",sep,
                   "sdx(m)",sep,"sdy(m)",sep,"sdz(m)",sep,"sdxy(m)",sep,
                   "sdyz(m)",sep,"sdzx(m)",sep,"sdvx(m/s)",sep,"sdvy(m/s)",sep,
                   "sdvz(m/s)",sep,"sdvxvy(m/s)",sep,"sdvyvz(m/s)",sep,"sdvzvx(m/s)",sep,"age(s)",sep,"ratio");
    }
