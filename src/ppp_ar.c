                       ...
                       const nav_t *nav, const double *azel)
{
    ...
    for (i=0;i<n;i++) {
        ...
        fprintf(output, "satellite %i: EW = %lf, var = %lf", sat, EW, var);
        ...
        if (amb->n[1]<=0) {
            ...
            fprintf(output, "amb->LC[1] = %lf, amb->LCv[1] = %lf, amb->n[1] = %i\n", amb->LC[1], amb->LCv[1], amb->n[1]);
        }
                      ...
                      const nav_t *nav, const double *azel)
{
    ...
    for (i=0;i<n;i++) {
        ...
        var=SQR(0.7*rtk->opt.eratio[0])*
            (SQR(rtk->opt.err[1])+SQR(rtk->opt.err[2]/sin(azel[1+2*i])));
        fprintf(output, "satellite %i: MW = %lf, var = %lf", sat, MW, var);
        ...
        if (amb->n[0]<=0) {
            ...
            fprintf(output, "amb->LC[0] = %lf, amb->LCv[0] = %lf, amb->n[0] = %i\n", amb->LC[0], amb->LCv[0], amb->n[0]);
        }
                  ...
                  const nav_t *nav, const double *azel, double *x, double *P)
{
    ...
    double ep[6];FILE* out = fopen("/home/user/ws/function_calls.txt", "a");
    fprintf(out, "ppp_ar\n");
    fclose(out);
    time2epoch(obs->time, ep);
    output = fopen("/home/user/ws/ppp_ar.txt", "a");
    fprintf(output, "\nAmbiguity resolution for time %.0f/%.0f/%.0f %.0f:%.0f:%.4f\n", ep[0], ep[1], ep[2], ep[3], ep[4], ep[5]);

    
    /* ambiguity resolution by WL/NL for iono-free LC */
    if (rtk->opt.ionoopt==IONOOPT_IFLC) {
        return ppp_amb_IFLC(rtk,obs,n,exc,nav,azel,x,P);fclose(output);
    }
    /* ambiguity resolution by ILS */
    else {
        return ppp_amb_ILS(rtk,obs,n,exc,nav,azel,x,P);fclose(output);
    }
}
